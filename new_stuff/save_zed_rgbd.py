#!/usr/bin/env python3
#python save_zed_rgbd.py --capture_hz 30 --resize_width 640 --resize_height 360
# for the pipeline,  to store a single frame --max-frames 1
...
class ZedRGBDDataSaver(Node):
    def __init__(
        ...
        super().__init__("zed_rgbd_data_saver")
        ...
        self.save_dir = os.path.join(self.save_root, "zed")
        ...
        if self.max_frames is None:
            self.get_logger().info("Capture mode: continuous")
        else:
            self.get_logger().info(f"Capture mode: stop after {self.max_frames} valid frame(s)")
    ...
    def save_frame(self):
        ...
        if self.max_frames is not None and self.frame_id >= self.max_frames:
            self.get_logger().info(
                f"Reached max_frames={self.max_frames}. Stop requested."
            )
...
def main():
    ...
    parser.add_argument(
        "--save_dir",
        type=str,
        default="/docker_shared",
        help="Root directory where rgb/depth/camera folders are saved.",
    )
    parser.add_argument(
        "--rgb_topic",
        type=str,
        default="/zed_down/zed_node/rgb/color/rect/image",
    )
    parser.add_argument(
        "--depth_topic",
        type=str,
        default="/zed_down/zed_node/depth/depth_registered",
    )
    parser.add_argument(
        "--camera_info_topic",
        type=str,
        default="/zed_down/zed_node/rgb/color/rect/camera_info",
    )
    parser.add_argument(
        "--max_rgb_depth_dt",
        type=float,
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Maximum number of valid RGB-D frames to save. "
            "If omitted, capture continues until stopped."
        ),
    )
    ...
    if args.max_frames is not None and args.max_frames <= 0:
        parser.error("--max_frames must be greater than 0")

    rclpy.init()

    node = ZedRGBDDataSaver(
