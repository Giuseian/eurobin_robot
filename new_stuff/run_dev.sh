...
# Re-use existing container (avviandolo prima se risulta fermo, invece di ricrearlo).
if [ "$(docker ps -a --quiet --filter name=$CONTAINER_NAME)" ]; then
    if [ -z "$(docker ps --quiet --filter status=running --filter name=$CONTAINER_NAME)" ]; then
...
docker run -it \
    ...
    -v /mnt/data/kyon/docker_shared:/docker_shared \
    ...
    $BASE_NAME \
    /bin/bash -c '
      echo "=== Setup automatico del nuovo container ===";
      sudo apt-get update;
      rosdep update;
      rosdep install --from-paths src/isaac_ros_pose_estimation/isaac_ros_foundationpose --ignore-src -y;
      sudo apt-get install -y ros-humble-isaac-ros-examples;
      grep -qxF "source /workspaces/isaac_ros-dev/install/setup.bash" ~/.bashrc || echo "source /workspaces/isaac_ros-dev/install/setup.bash" >> ~/.bashrc;
