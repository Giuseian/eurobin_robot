...
class Config:
    ...
    robot_docker_dir: Path = Path.home() / "Documents" / "PROJECTS" / "eurobin_robot" / "robot_docker"
    perception_docker_dir: Path = Path.home() / "Documents" / "PROJECTS" / "eurobin_robot" / "perception_docker"
    ...
    robot_homing_script: str = "/home/user/eurobin/manipulation_code/centauro/homing_centauro.py"
...
def should_run_homing_after_action(action: str) -> bool:
    """Return True when the robot should go back to home after a planned action.

    Grasp is intentionally excluded because after a grasp the robot should keep
    holding the object for the post-condition validation.
    """
    return normalize_robot_action_name(action) != "grasp"


def run_homing_centauro(cfg: Config) -> None:
    print()
    print("[robot] Launching homing_centauro.py inside robot_docker...")
    print("[robot] Working directory:")
    print(f"        {cfg.robot_manipulation_dir}")
    print("[robot] Script:")
    print(f"        {cfg.robot_homing_script}")
...
test -f {shlex.quote(cfg.robot_homing_script)} || {{
  echo 'Error: homing script not found: {cfg.robot_homing_script}' >&2
  exit 1
}}
...
echo ''
echo 'Environment used for homing execution:'
echo "USER=$(whoami)"
echo "PWD=$(pwd)"
echo "CONDA_DEFAULT_ENV=${{CONDA_DEFAULT_ENV:-}}"
echo "VIRTUAL_ENV=${{VIRTUAL_ENV:-}}"
...
python3 -u {shlex.quote(cfg.robot_homing_script)}
"""
...
def normalize_robot_action_name(action: str) -> str:
...
def find_planned_object_result(
    object_results: list[dict[str, Any]],
    planned_object: str,
) -> dict[str, Any]:
    target_key = normalize_object_name(planned_object)

    for result in object_results:
        object_name = str(result.get("object_name", ""))
        if normalize_object_name(object_name) == target_key:
            return result

    for result in object_results:
        prompt = str(result.get("prompt", ""))
        if normalize_object_name(prompt) == target_key:
            return result
    ...
    for result in object_results:
        ...
        for candidate in candidates:
            candidate_key = normalize_object_name(candidate)
            if target_key and (target_key in candidate_key or candidate_key in target_key):
                return result

    available = [
        f"{result.get('object_name', '(unknown)')} / prompt={result.get('prompt', '(none)')}"
        for result in object_results
    ]

    raise RuntimeError(
        f"Planned object {planned_object!r} was not found in FoundationPose results. "
        f"Available objects: {available}"
    )


def execute_planned_robot_action(
    cfg: Config,
    object_results: list[dict[str, Any]],
    planned_action: str,
    planned_object: str,
    ...
    action = normalize_robot_action_name(planned_action)
    selected = find_planned_object_result(object_results, planned_object)
    pose = selected["pose"]

    print()
    print("============================================================")
    print("[action] Automatic planned robot action")
    if stage_id is not None:
        print(f"[action] Stage:          {stage_id}")
    print(f"[action] Planned action: {planned_action}")
    print(f"[action] Normalized:     {action}")
    print(f"[action] Planned object: {planned_object}")
    print(f"[action] Matched object: {selected.get('object_name')}")
    print(f"[action] Prompt:         {selected.get('prompt', selected.get('object_name'))}")
    print(f"[action] Pose:           {pose.robot_object_position_line}")
    print("============================================================")

    if action not in {"grasp", "push", "place"}:
        raise RuntimeError(
            f"Unsupported planned action {planned_action!r}. "
            "Supported actions are: grasp, push. Place is recognized but not implemented yet."
        )

    if action == "place":
        raise RuntimeError(
            "Planned action 'place' was requested, but no automatic place controller "
            "is configured in this pipeline yet."
        )
    ...
    if action == "grasp":
        ...
        if grasp_box_width is None:
            raise RuntimeError(
                f"Cannot execute planned grasp for {selected.get('object_name')}: "
                "grasp_box_width is not available. Check mesh_metadata.json."
            )

        run_grasp_centauro(cfg, box_width=float(grasp_box_width))
        print("[action] Planned grasp action completed.")

    elif action == "push":
        run_push_box_xyz(cfg)
        print("[action] Planned push action completed.")

    if should_run_homing_after_action(action):
        print()
        print("============================================================")
        print(f"[action] Running homing after planned action: {action}")
        print("============================================================")
        run_homing_centauro(cfg)
    else:
        print()
        print("============================================================")
        print("[action] Skipping homing after grasp action.")
        print("[action] The robot remains in the grasp state for post-validation.")
        print("============================================================")
...
def run_vlm_task_validation_with_auto_capture(
    ...
    print("[6/8] Automatic robot action execution is enabled.")
    print("[6/8] live2 will ask you to approve the final executable plan.")
    print("[6/8] After each matching PRE condition, live2 emits ACTION_REQUEST.")
    print("[6/8] This host pipeline executes the requested action and captures the next RGB-D frame.")
...
"""
    ...
    action_request_prefix = "[ACTION_REQUEST]"
    ...
    def capture_after_planned_action(capture_index: int) -> None:
        print()
        print("============================================================")
        print(f"[auto-capture] Capturing RGB-D frame #{capture_index} from robot_docker...")
        print("============================================================")
    ...
    def handle_action_request(payload_text: str) -> None:
        nonlocal action_in_progress
        nonlocal action_count

        if action_in_progress:
            print("[action] Ignoring ACTION_REQUEST because another action is already in progress.")
            return

        try:
            payload = json.loads(payload_text)
        except json.JSONDecodeError as exc:
            raise RuntimeError(f"Invalid ACTION_REQUEST JSON: {payload_text}") from exc

        if not isinstance(payload, dict):
            raise RuntimeError(f"ACTION_REQUEST payload must be a JSON object: {payload!r}")

        stage_id_raw = payload.get("stage_id")
        planned_action = payload.get("action")
        planned_object = payload.get("object")

        if not isinstance(stage_id_raw, int):
            raise RuntimeError(f"ACTION_REQUEST has invalid stage_id: {stage_id_raw!r}")
        if not isinstance(planned_action, str) or not planned_action.strip():
            raise RuntimeError(f"ACTION_REQUEST has invalid action: {planned_action!r}")
        if not isinstance(planned_object, str) or not planned_object.strip():
            raise RuntimeError(f"ACTION_REQUEST has invalid object: {planned_object!r}")
        ...
        print()
        print("============================================================")
        print(f"[action] Received ACTION_REQUEST #{action_count}.")
        print(f"[action] Stage:  {stage_id_raw}")
        print(f"[action] Action: {planned_action}")
        print(f"[action] Object: {planned_object}")
        print("============================================================")

        try:
            execute_planned_robot_action(
                cfg=cfg,
            ...
            capture_after_planned_action(action_count)
    ...
    def on_live2_line(line: str) -> None:
        ...
        if not stripped.startswith(action_request_prefix):
            return

        payload_text = stripped[len(action_request_prefix):].strip()
        handle_action_request(payload_text)
